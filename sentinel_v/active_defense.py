...
class ActiveDefenseEngine:
    ...
    def _save_action(self, action: str, target: str, reason: str, success: bool) -> None:
    ...
    def _is_internal_ip(self, ip: str) -> bool:
        """Check if an IP is in the internal network ranges."""
        try:
    ...
    def _block_ip_nftables(self, ip: str, duration: Optional[int]) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                subprocess.run(["nft", "add", "table", "inet", "sentinel_v"], check=True)
                subprocess.run(["nft", "add", "chain", "inet", "sentinel_v", "input", "{ type filter hook input priority 0 ; }"], check=True)
                subprocess.run(["nft", "add", "chain", "inet", "sentinel_v", "output", "{ type filter hook output priority 0 ; }"], check=True)
            
            # Add block rule
            subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "input", f"ip saddr {ip} counter drop"], check=True)
            subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "output", f"ip daddr {ip} counter drop"], check=True)
    ...
    def _block_ip_windows(self, ip: str, duration: Optional[int]) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                f"action=block",
                f"remoteip={ip}",
                f"description=Blocked by Sentinel-V"
            ]
            subprocess.run(cmd, check=True)
    ...
    def _block_ip_macos(self, ip: str, duration: Optional[int]) -> bool:
        ...
        try:
            ...
            result = subprocess.run(["pfctl", "-sr"], capture_output=True, check=False)
    ...
    def unblock_ip(self, ip: str) -> bool:
        ...
        try:
            if self.platform == "linux":
                if self._firewall_tool == "nftables":
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "input", f"ip saddr {ip} counter drop"], check=False)
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "output", f"ip daddr {ip} counter drop"], check=False)
                else:
                    subprocess.run(["iptables", "-D", "INPUT", "-s", ip, "-j", "DROP"], check=False)
                    subprocess.run(["iptables", "-D", "OUTPUT", "-d", ip, "-j", "DROP"], check=False)
            elif self.platform == "windows":
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Block-{ip}"], check=False)
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Block-{ip}-OUT"], check=False)
            elif self.platform == "darwin":
    ...
    def _execute_block_network(self, network: str, duration: Optional[int]) -> bool:
        ...
        try:
            if self.platform == "linux":
                if self._firewall_tool == "nftables":
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "input", f"ip saddr {network} counter drop"], check=True)
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "output", f"ip daddr {network} counter drop"], check=True)
                else:
                    subprocess.run(["iptables", "-A", "INPUT", "-s", network, "-j", "DROP"], check=True)
                    subprocess.run(["iptables", "-A", "OUTPUT", "-d", network, "-j", "DROP"], check=True)
            elif self.platform == "windows":
                subprocess.run([
                    "netsh", "advfirewall", "firewall", "add", "rule",
                    f"name=Sentinel-V-Block-Network-{network.replace('/', '-')}",
                    f"dir=in",
                    f"action=block",
                    f"remoteip={network}",
                    f"description=Blocked by Sentinel-V"
                ...
                subprocess.run([
                    "netsh", "advfirewall", "firewall", "add", "rule",
                    f"name=Sentinel-V-Block-Network-{network.replace('/', '-')}-OUT",
                    f"dir=out",
                    f"action=block",
                    f"remoteip={network}",
                    f"description=Blocked by Sentinel-V"
    ...
    def unblock_network(self, network: str) -> bool:
        ...
        try:
            if self.platform == "linux":
                if self._firewall_tool == "nftables":
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "input", f"ip saddr {network} counter drop"], check=False)
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "output", f"ip daddr {network} counter drop"], check=False)
                else:
                    subprocess.run(["iptables", "-D", "INPUT", "-s", network, "-j", "DROP"], check=False)
                    subprocess.run(["iptables", "-D", "OUTPUT", "-d", network, "-j", "DROP"], check=False)
            elif self.platform == "windows":
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Block-Network-{network.replace('/', '-')}"], check=False)
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Block-Network-{network.replace('/', '-')}-OUT"], check=False)
            elif self.platform == "darwin":
                try:
                    with open("/etc/pf.anchors/com.apple/250.Sentinel-V", "r") as f:
                        lines = f.readlines()
            ...
            self._save_action("unblock_network", network, "Manual or timeout removal", True)
            return True
        except Exception as e:
    ...
    def _isolate_via_vlan(self, ip: str, vlan: str) -> bool:
        ...
        logging.info(f"Would move {ip} to VLAN {vlan} (requires network hardware integration)")
        return True
        ...
        try:
            if self.platform == "linux":
                if self._firewall_tool == "nftables":
                    ...
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "input", f"ip saddr {ip} counter drop"], check=True)
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "input", f"ip daddr {ip} counter drop"], check=True)
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "output", f"ip saddr {ip} counter drop"], check=True)
                    subprocess.run(["nft", "add", "rule", "inet", "sentinel_v", "output", f"ip daddr {ip} counter drop"], check=True)
                else:
                    subprocess.run(["iptables", "-A", "INPUT", "-s", ip, "-j", "DROP"], check=True)
                    subprocess.run(["iptables", "-A", "INPUT", "-d", ip, "-j", "DROP"], check=True)
                ...
                subprocess.run([
                    ...
                    f"action=block",
                    f"remoteip={ip}",
                    f"description=Isolated by Sentinel-V"
                ], check=True)
                subprocess.run([
                    "netsh", "advfirewall", "firewall", "add", "rule",
    ...
    def unisolate_host(self, ip: str) -> bool:
        ...
        try:
            if self.platform == "linux":
                if self._firewall_tool == "nftables":
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "input", f"ip saddr {ip} counter drop"], check=False)
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "input", f"ip daddr {ip} counter drop"], check=False)
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "output", f"ip saddr {ip} counter drop"], check=False)
                    subprocess.run(["nft", "delete", "rule", "inet", "sentinel_v", "output", f"ip daddr {ip} counter drop"], check=False)
                else:
                    subprocess.run(["iptables", "-D", "INPUT", "-s", ip, "-j", "DROP"], check=False)
                    subprocess.run(["iptables", "-D", "INPUT", "-d", ip, "-j", "DROP"], check=False)
                    subprocess.run(["iptables", "-D", "OUTPUT", "-s", ip, "-j", "DROP"], check=False)
                    subprocess.run(["iptables", "-D", "OUTPUT", "-d", ip, "-j", "DROP"], check=False)
            elif self.platform == "windows":
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Isolate-{ip}"], check=False)
                subprocess.run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Sentinel-V-Isolate-{ip}-OUT"], check=False)
            elif self.platform == "darwin":
                try:
                    with open("/etc/pf.anchors/com.apple/250.Sentinel-V", "r") as f:
                        lines = f.readlines()
                    with open("/etc/pf.anchors/com.apple/250.Sentinel-V", "w") as f:
                        for line in lines:
                            if f"block from {ip}" not in line and f"block from any to {ip}" not in line:
                                f.write(line)
                    subprocess.run(["pfctl", "-f", "/etc/pf.conf"], check=True)
                except Exception:
                    pass
    ...
    def rate_limit(
        ...
        for rule in self.rate_limit_rules:
            if rule.source == source and rule.port == port and rule.protocol == protocol:
                return True
        ...
        if success:
            ...
            self._save_action("rate_limit", f"{source}:{port}/{protocol}", reason, False)
        
        return success

    def _execute_rate_limit(self, source: str, port: int, protocol: str, rate: str) -> bool:
        ...
        try:
            ...
            logging.error(f"Error applying rate limit to {source}:{port}/{protocol}: {e}")
            return False

    def _rate_limit_linux(self, source: str, port: int, protocol: str, rate: str) -> bool:
        ...
        try:
            ...
            if protocol == "tcp":
                cmd = [
                    ...
                    "--dport", str(port), "-s", source,
                    "-m", "recent", "--set", "--name", f"SENTINEL-RL-{source}-{port}",
                    "--rsource"
                ]
                subprocess.run(cmd, check=True)
                ...
                cmd = [
                    "iptables", "-A", "INPUT", "-p", "tcp",
                    "--dport", str(port), "-s", source,
                    "-m", "recent", "--update", "--seconds", str(time_seconds),
                    "--hitcount", str(count), "--name", f"SENTINEL-RL-{source}-{port}",
                    "--rsource", "-j", "DROP"
                ]
                subprocess.run(cmd, check=True)
    ...
    def _rate_limit_windows(self, source: str, port: int, protocol: str, rate: str) -> bool:
    ...
    def _rate_limit_macos(self, source: str, port: int, protocol: str, rate: str) -> bool:
        ...
        try:
            ...
            rule = f"table <sentinel_rl_{source}_{port}> {{ {source} }}"
            with open("/etc/pf.anchors/com.apple/250.Sentinel-V", "a") as f:
                f.write(rule + "\n")
                f.write(f"pass in proto {protocol} from <sentinel_rl_{source}_{port}> to port {port} keep state (max-src-conn-rate {count}/{time_seconds}, overload <sentinel_rl_{source}_{port}_over>)\n")
                f.write(f"block in proto {protocol} from <sentinel_rl_{source}_{port}_over> to port {port}\n")
    ...
    def remove_rate_limit(
        ...
        self._save_action("remove_rate_limit", f"{source}:{port}/{protocol}", "Manual removal", True)
        return True
